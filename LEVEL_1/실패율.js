...
function solution(N, stages) {
    ...
    for (let index = 1; index <= N; index++) {
        ...
        answer[index - 1] = {
            rate: stages.length !== 0 ? answer[index - 1] / stages.length : 0,
            stage: index,
        };
        stages = stages.filter((stage) => stage !== index);
    }

    answer = answer.sort(sortCallBack).map((value) => value.stage);
...
testCases.forEach((testCase) => {
    console.log(
        JSON.stringify(solution(testCase.N, testCase.stages)) ===
